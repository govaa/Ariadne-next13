...
var prefixInTag = function prefixInTag(range, tag) {
  return tag.toLowerCase().indexOf("".concat(range, "-").toLowerCase()) !== -1;
};
...
var rangeSort = function rangeSort(a, b) {
  var value = ~~(isWildTag(b) || gteQty(b, a) || !isWildTag(a) && qtyMatch(a, b) && lengthSort(a.tag, b.tag));
  if (value > 0) return 1;
  if (value === 0) return -1;
  return 0;
};
...
function languagePriorityList(range) {
  function extractTag(rangeTag) {
    ...
    return {
      tag: matches[2] ? [matches[1], matches[2]].join('-') : matches[1],
      quality: extractQuality(matches[3])
    };
  }
...
function lookup(range, languageTags, defaultValue) {
  ...
  var sortedLangTags = languageTags.sort(lengthSort);
